namespace InternController {
	const myApp = angular.module('myApp');
	myApp.controller('InternCtrl', ['$scope','$http', '$timeout', function($scope, $http, $timeout, $location) {
		...
		$scope.addIntern = function() {
			...
			if (!$scope.intern.points) {
				...
				if (/^\d+$/.test($scope.intern.points)) {
					$scope.intern.points = Number($scope.intern.points);
				}
				else {
					$scope.showError = true;
					$scope.errorTitle = 'Points Parse Error';
					$scope.errorMessage = 'Make sure points is a positive whole number';
					$timeout(function(){
					$scope.showError = false;
					}, 4500);
					return;
		...
		$scope.subOne = function(id, intern) {
			if (intern.points - 1 < 0) {
				$scope.showError = true;
				$scope.errorTitle = 'Negative Points Error';
				$scope.errorMessage = 'Too few points! Be nice.';
				$timeout(function(){
				$scope.showError = false;
				}, 4500);
				return;
